...
// Fonction qui teste si un fichier existe déjà ou non
...
bool XLaserTargetProcess::ProcessFile(std::string filename)
{
	...
	if (cas.compare("cible") == 0)
	{
		...
		// On copie les points pour générer un vecteur contenant que la géométrie des points
		for (uint32 i = 0; i < (*iter).second.size(); i++)
		{
		...
		// Détermination de l'équation du plan 
		...
		// Creation d'une base orthonormée pour traiter le nuage dans le plan local
		detection.constructionBase();

		// Génération des matrices de passage
		...
		// Dimension terrain d'un pixel <=> Résolution
		...
		// On crée l'orthoimage que si elle n'existe pas déjà (évite des temps de calculs)		
		if (!exists_test(file))
		{
			try {
				// Détermination du nombre de ligne et de colonne nécessaire pour écrire l'orthoimage
				...
				for (int ligneEnCours = 0; ligneEnCours < ligne; ligneEnCours++)
				{
					for (int colonneEnCours = 0; colonneEnCours < colonne; colonneEnCours++)
					{
						...
						// Si un point correspond on normalise son intensité et on rajoute son intensite dans l'ortho
						if (indicePointAAjouter != -1)
						{
							...
							*pix = (unsigned char)floor((p1 + abs(minIntensite))*ratio);
							//// Correction si intensité minimale pour ne pas apparaitre comme un pixel noir
		...
		// Correlation avec la référence
		std::string reference = m_params.output_path + image2D; // Image à retrouver
		try {
	...
	if (cas.compare("sphere") == 0)
	{
		...
		if ((*iter).second.size() > 120) //if (points_color.size() > 120)
		{
			...
			while (diff > 1e-15)
			{
				...
				// Si trop long
				if (loop > 30){
					XErrorInfo(m_error, __FUNCTION__, "Itération trop importante -> conditions initiales erronnées");
					break;
				}
			}
		}
	...
	if (xr!=0)
	{
		...
		if (fichierXYZ)  // si l'ouverture a réussi
		{
			bool etat = false;
			int nb_ligne = 0;
			// instructions
			// si elt déjà écrit alors on sort
			std::string ligne;
			...
			fichierXYZ.seekg(0, std::ios::end);  //On se déplace à la fin du fichier		
			if (etat == true){
				fichierXYZ.close();
			}
			// sinon on va a la fin du fichier et on écrit
			else{
		...
		else  // sinon
		{
			...
			std::ofstream monflux(nom.c_str()); //déclaration du flux et ouverture du fichier 
			if (monflux)
			{
				//remplir l'entête
				...
				XErrorError(m_error, __FUNCTION__, "Erreur dans l'écriture XYZ");
			}
		}
	}
	else
