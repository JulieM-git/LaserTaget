...
std::vector<std::string> XStringTools::Tokenize(const std::string &str, char sep, bool multiSepAsOne)
{
	...
	while (getline(is, token, sep)){
		if(multiSepAsOne){//on considère plusieurs séparateurs consécutifs comme un seul
			if(!token.length())
				continue;
		}
		vec.push_back(token);
...
//-----------------------------------------------------------------------------
//Décode un booléen dans une chaine de caractère
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//Convertit une string en uint32 en concatenant tous les caractères numériques contenus dans la chaîne
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//Extrait une valeur entière dans une string.à partir du début
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//Extrait une valeur entière dans une string.à partir de la fin
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
//Retire les blancs en début et fin de de chaine
//-----------------------------------------------------------------------------
std::string XStringTools::RemoveBlanks(std::string s)
{
  for(unsigned int i=0; i<s.size(); i++)
  {
    if(s[i] != ' ' && s[i] != '\t')//premier caractère non blanc
	{
	for(unsigned int j=(int)(s.size())-1;j>=i;j--)
	  {
		if(s[j] != ' ' && s[j] != '\t')//dernier caractère non blanc
		{
...
//-----------------------------------------------------------------------------
//méthode commune a différents fichiers texte permettant de considérer une ligne comme un commentaire
//une ligne de commentaire commence par une * ( comp3D par exemple)
//un commentaire comence par // (entete géoconcept par exemple)
//si le premier caractère rencontré est différent de " \t*" la ligne est valide
bool XStringTools::IdentifyLineAsComment(std::string& ligne)
{
	//on cherche le premier caractère différent de ' 'et '\t' 
  for(unsigned int i=0; i<ligne.size();i++)
  {
    if(ligne[i] == ' ' || ligne[i] == '\t' )
		continue;
	//test du premier caractère valide
    if(ligne[i] == '*' || ligne[i] == '/' )
		return true;
	 return false;
  }
  return true;// aucun caractère valide dans la ligne
}
//-----------------------------------------------------------------------------
