...
/*
NOTE:
L'écriture de données dans le fichier de log se fait en effectuant un append dans le fichier
juste avant le tag de fin...

Cette opération qui peut sembler extrêmement contre performante ne l'est pas
car le fichier est alors entièrement en cache du disque

Cependant, si le fichier de log prend une taille considérable ça peut être lent

Par contre, le disque du coup se retrouve trés sollicité à la fois en lecture et
écriture...

Il ne faut donc pas trop le faire bosser en même temps
*/
...
void XErrorXml::WriteToOutput(std::ostream* out)
{
	...
	m_log->seekp((uint32)m_log->tellp() - (uint32)m_tagEnd.length());
	//TODO A VOIR  compilation VS13 *m_log << *out << m_tagEnd;
