...
//-----------------------------------------------------------------------------
// Renvoie le nom du repertoire parent (vide si pas de r�pertoire)
//-----------------------------------------------------------------------------
...
{//*****non test� !!!
	...
	int pos = (int) P.rfind('.'); 
	if(pos != std::string::npos)//on enl�ve le nom de fichier
...
//-----------------------------------------------------------------------------
//renvoie le repertoirede niveau  imm�diatement sup�rieur
//-----------------------------------------------------------------------------
std::string XPath::UpFolder(const char* folder_name)
{//*****non test� !!!
	std::string P = folder_name;
	SubPathSep(P);
	return P.substr(P.rfind(PathSep)+1);
}
//-----------------------------------------------------------------------------
...
std::string XPath::NameNoExt(const char* filename)
{
	...
	return P.substr(0,P.rfind('.'));//enl�ve les doubles extensions (.ori.xml par exemple)
}
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Renvoie la premi�re extension d'un fichier
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Renvoie l'extension compl�te d'un fichier (ori.xml)
...
//-----------------------------------------------------------------------------
// change l'extension ou ajoute  une extension � un fichier
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// ins�re une chaine dans le nom du fichier avant l'extension 
...
{
	std::string R = root;
	SubPathSep(R);//root ne doit pas avoir le \ � la fin
...
//-----------------------------------------------------------------------------
// Renvoie un chemin relatif dans le cas o� path est un repertoire
//� utiliser pour �tre sur d'avoir le \ � la fin -> ex : ".\orthos_imagettes\"
//-----------------------------------------------------------------------------
std::string XPath::FolderRelative(const char* root, const char* path)
{
	...
	// path = C:\toto\titi
        // => .\titi
	//�� c'est ok lorsque le repertoire est un sous repertoire, on a bien le \ a la fin
	pos = (int)P.find(R);
	if (pos == 0) {
		if (P[R.length()] == PathSep) {//pour v�rifier le cas e:\toto  e:\tototiti\tata
			Rel = "." + P.substr(R.length());
			return Rel;
		}
	}
	...
	//�� c'�tait pas bon dans la fonction "relative", on avait pas le \ a la fin
	std::string root1 = R.substr(0, R.find(PathSep));
	std::string root2 = P.substr(0, P.find(PathSep));
	if (root1.compare(root2) != 0)	// Pas le meme disque
		return P;
...
std::string XPath::Absolute(const char* root, const char* path)
{
	...
	//ajout jpp pour compatibilit� avec ancienne version qui rajoutait un \ en trop
...
//� utiliser quand on est pas s�r d'avoir le \ � la fin -> ex : ".\orthos_imagettes\"
//-----------------------------------------------------------------------------
